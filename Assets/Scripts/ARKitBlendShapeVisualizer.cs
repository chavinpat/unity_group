...
namespace UnityEngine.XR.ARFoundation.Samples
{
    ...
    public class ARKitBlendShapeVisualizer : MonoBehaviour
    {
        ...
        private ARKitBlendShapeLocation blendShapeToTrack_tongue = ARKitBlendShapeLocation.TongueOut;
        public float jaw_coef = 0f;
        public float tongue_coef = 0f;
        // public Dictionary<ARKitBlendShapeLocation, float> cacheBlendShape;
        ...
        void UpdateFaceFeatures()
        {
            ...
            var cacheBlendShape = new Dictionary<ARKitBlendShapeLocation, float>();

            using (var blendShapes = m_ARKitFaceSubsystem.GetBlendShapeCoefficients(m_Face.trackableId, Allocator.Temp))
            {
                // Debug.Log(blendShapes[24].coefficient);
                // Debug.Log(blendShapes[24].blendShapeLocation);
                // Debug.Log(blendShapes[5].coefficient);
                // Debug.Log(blendShapes[5].blendShapeLocation);  
                foreach (var featureCoefficient in blendShapes)
                {
                    cacheBlendShape[featureCoefficient.blendShapeLocation] = featureCoefficient.coefficient;
                    // Debug.Log("cacheBlendShape = "+ cacheBlendShape);
                    // Debug.Log($"{featureCoefficient.blendShapeLocation}: {featureCoefficient.coefficient}");
                ...
                Debug.Log("m_FaceArkitBlendShapeIndexMap = "+ m_FaceArkitBlendShapeIndexMap);
                jaw_coef = cacheBlendShape[blendShapeToTrack];
                tongue_coef = cacheBlendShape[blendShapeToTrack_tongue];
                Debug.Log("jaw_coef = "+ jaw_coef);
                Debug.Log("tongue_coef = "+ tongue_coef);
                playerMovement.GetCoeff (jaw_coef,tongue_coef);
            }
    #endif
        }
    }
